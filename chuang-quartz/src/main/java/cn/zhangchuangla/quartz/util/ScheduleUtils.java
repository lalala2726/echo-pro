...
public class ScheduleUtils {
    ...
    private static Class<? extends Job> getQuartzJobClass(SysJob sysJob) {
        boolean isConcurrent = QuartzConstants.JOB_CONCURRENT_ALLOWED.equals(sysJob.getConcurrent());
    ...
    public static CronScheduleBuilder handleCronScheduleMisfirePolicy(SysJob job, CronScheduleBuilder cb)
        ...
        return switch (job.getMisfirePolicy()) {
            case ScheduleConstants.MISFIRE_DEFAULT -> cb.withMisfireHandlingInstructionDoNothing();
            ...
            default -> throw new Exception("The task misfire policy '" + job.getMisfirePolicy()
                    + "' cannot be used in cron schedule tasks or is not supported.");
        };
    }
    ...
    public static boolean whiteList(String invokeTarget) {
        String packageName = "";
        int index = invokeTarget.lastIndexOf(".");
        if (index != -1) {
            packageName = invokeTarget.substring(0, index);
        }
        // 白名单验证
        for (String allow : QuartzConstants.INVOKE_TARGET_WHITELIST_PACKAGES) {
            if (packageName.startsWith(allow)) {
                return false;
            }
        }
        return true;
